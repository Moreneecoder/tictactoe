...
loop do
    ...
    if board.is_winning_combo?(player1.token)
        abort("GAME OVER! #{player1.name.upcase} WINS GAME!")
    end

    # CHECK IF STALEMATE
    if board.is_stalemate?
        abort("TOUGH GAME! THERE WAS NO WINNER")
    end
    ...
    #CHECK IF WINNING COMBO
    if board.is_winning_combo?(player2.token)
        abort("GAME OVER! #{player2.name.upcase} WINS GAME!")
    end
